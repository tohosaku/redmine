...
suite('context menu', () => {
  ...
  suite('when a row is clicked', async () => {
    ...
    setup(() => {
      ...
      rows = document.querySelectorAll('.hascontextmenu');
    });
    ...
    test('When the checkbox is clicked directly, select the row', () => {
      const target = rows[0].querySelector('input');
      target.checked = true;
      const tr       = target.closest('.hascontextmenu');
      selectRows(target, tr, {} )
      assert.isTrue(tr.classList.contains('context-menu-selection'));
    });

    test('When the td containing the checkbox is clicked, toggle the checkbox and select the row', () => {
      const target = rows[0].querySelector('td.checkbox');
      const tr     = target.closest('.hascontextmenu');
      selectRows(target, tr, {} )
      assert.isTrue(target.querySelector('input').checked);
      assert.isTrue(tr.classList.contains('context-menu-selection'));
    });

    test('When the td not containing the checkbox is clicked with no modifier, toggle the checkbox and select the row', () => {
      const target0 = rows[0].querySelector('td.noinput');
      const tr0     = target0.closest('.hascontextmenu');
      selectRows(target0, tr0, {} )
      assert.isTrue(tr0.querySelector('input').checked);
      assert.isTrue(tr0.classList.contains('context-menu-selection'));

      const target1 = rows[1].querySelector('td.noinput');
      const tr1     = target1.closest('.hascontextmenu');
      selectRows(target1, tr1, {} )

      assert.isFalse(tr0.querySelector('input').checked);
      assert.isFalse(tr0.classList.contains('context-menu-selection'));

      assert.isTrue(tr1.querySelector('input').checked);
      assert.isTrue(tr1.classList.contains('context-menu-selection'));
    });
    ...
    test('When the td not containing the checkbox is clicked with ctrl key, toggle the checkbox and select the row', () => {
      ...
      const target3 = rows[3].querySelector('td.noinput');
      const tr3     = target3.closest('.hascontextmenu');
      selectRows(target3, tr3, {ctrlKey:true} )

      assert.isTrue(tr0.querySelector('input').checked);
      assert.isTrue(tr0.classList.contains('context-menu-selection'));

      assert.isTrue(tr3.querySelector('input').checked);
      assert.isTrue(tr3.classList.contains('context-menu-selection'));
    });
  });
  ...
  suite('get context menu position', () => {
    ...
    test('reverseRenderAction returns function to change DOM Element', () => {
      ...
      action(element);
      assert.isTrue(element.classList.contains('reverse-x'));
      assert.equal('50px', element.style.left);
    });
    ...
    test('normalRenderAction returns function to change DOM Element', () => {
      ...
      action(element);
      assert.isFalse(element.classList.contains('reverse-x'));
      assert.equal('100px', element.style.left);
    });
    ...
    test('reverseFolderAction returns funtion to change submenu element', () => {
      ...
      assert.isTrue(Array.from(folders).every(e => e.classList.contains('down')));
    });
    ...
    test('normalFolderAction returns funtion to change submenu element', () => {
      ...
      action(element);
      const folders = element.querySelectorAll('.folder');
      assert.isTrue(Array.from(folders).every(e => e.classList.contains('up')));
    });
