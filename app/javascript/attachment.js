 ...
 */
import { metaContent, updateSVGIcon } from "helper"
...
export default class Attachment {
  ...
  addInputFiles(inputEl) {
    ...
    if (this.isAjaxSupported(inputEl)) {
      ...
      if (fileSpan) {
        ...
        inputEl.removeAttribute('data-action');
  ...
  uploadAndAttachFiles(files, inputEl) {
    ...
    if (sizeExceeded) {
      throw new Error(inputEl.dataset.maxFileSizeMessage);
    } else {
      filesArray.forEach((file) => {
        const fileElement = this.addFile(inputEl, file, true);
        if (fileElement !== null) {
          this.ajaxUpload(file, fileElement.attachmentId, fileElement.fileSpan, inputEl);
        }
      });
    }

    if (filesLength > this.maxfilenumber) {
      throw new Error(inputEl.dataset.maxNumberOfFilesMessage);
    }
  ...
  get isFileNumExceeded() {
    return this.field.children.length >= this.maxfilenumber
  }
  ...
  ajaxUpload(file, attachmentId, fileSpan, inputEl) {
    ...
    const attachmentIcon = fileSpan.querySelector('svg.svg-attachment');

    function onLoadstart(e) {
      fileSpan.classList.remove('ajax-waiting');
      fileSpan.classList.add('ajax-loading');
      attachmentIcon.classList.add('svg-loader');
      updateSVGIcon(attachmentIcon, 'loader')
    ...
    const actualUpload = (file, attachmentId, fileSpan) => {
      ...
      this.uploadBlob(file, inputEl.dataset.uploadPath, attachmentId, {
        ...
        .finally(() => {
          ...
          attachmentIcon.classList.remove('svg-loader');
    ...
    updateSVGIcon(attachmentIcon, 'hourglass');
