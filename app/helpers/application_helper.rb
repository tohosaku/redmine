...
module ApplicationHelper
      ...
      tag.ul class: [:projects, {root: t.classes.empty?}], data: {controller: 'list', item_target: !t.classes.empty? && 'list'} do
        t.children.map do |child|
          ...
          tag.li class: child.classes, data: { controller: 'item', list_target: 'item' } do
            tag.div(class: child.classes){ yield(child.object, child.children.present?) } + child.content(self)
          end.html_safe
        end.join.html_safe
      end
    end
  ...
  def context_menu
    unless @context_menu_included
      ...
      content_for :context_menu do
        tag.div id: 'context-menu', style: 'display:none', data: {
          controller: 'context-menu', action: 'click@document->context-menu#hide contextmenu@document->context-menu#show change@document->context-menu#toggle'
        }
      end
  ...
  def datepicker_wrapper
    { data: { controller: 'datepicker' }}
  end

  def datepicker_input
    { data: { datepicker_target: 'input', action: 'click->datepicker#dispatch' }}
  end
  ...
  def copy_object_url_link(url)
    link_to(
      ...
      data: {'clipboard-text' => url, :controller => 'clipboard', :action => 'copyText' }
    )
  end
